...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-white" onMouseLeave={close}>
      {/* ── Top bar ── */}
      <div className="border-b border-gray-100">
        <div className="max-w-[1440px] mx-auto px-8 h-20 flex items-center flex-nowrap">
          ...
          <nav className="hidden lg:flex items-center flex-nowrap flex-1">
            {GNB.map((item) => (
              <button
                key={item.label}
                onMouseEnter={open}
                className="px-4 h-20 text-sm font-medium text-gray-700 hover:text-primary-700 transition-colors whitespace-nowrap"
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0 ml-auto">
            <button className="flex items-center gap-1 text-sm text-gray-500 border border-gray-200 rounded px-3 py-1.5 hover:border-gray-400 transition-colors">
          ...
          {/* Mobile hamburger */}
          <button onClick={() => setMobileOpen(true)} className="lg:hidden p-2 text-gray-600 ml-auto" aria-label="메뉴 열기">
