  ...
  return (
    <div className="fixed inset-0 z-50 bg-white overflow-y-auto lg:hidden">
      ...
      <nav className="px-4 py-2">
        {GNB.map((item) => (
          <AccordionItem key={item.label} item={item} depth={0} onClose={onClose} locale={locale} />
        ))}
      </nav>
      ...
      {open && item.children && (
        <div className={depth === 0 ? "bg-gray-50" : "bg-gray-100"}>
          {item.children.map((child) => (
            <AccordionItem key={child.label} item={child} depth={depth + 1} onClose={onClose} locale={locale} />
          ))}
        </div>
      )}
    </div>
