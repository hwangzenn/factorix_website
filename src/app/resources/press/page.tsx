...
export default async function PressPage() {
  const posts = await getPostsByTag("press");

  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <p className="text-sm text-primary-600 font-medium mb-2">자료실</p>
      <h1 className="text-4xl font-bold text-primary-800 mb-10">언론보도</h1>
      {posts.length === 0 ? (
        ...
        <ul className="divide-y divide-gray-100">
          {posts.map((post) => (
            <li key={post.id} className="py-5">
              <Link href={`/resources/press/${post.slug}`} className="font-medium text-gray-800 hover:text-primary-700 transition-colors">
                {post.title}
              </Link>
              {post.published_at && (
