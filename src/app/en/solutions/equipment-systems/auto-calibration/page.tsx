...
export const metadata: Metadata = {
  ...
  alternates: {
    ...
    languages: { ko: ROUTES.solutions.equipmentSystem.autoCalibration, en: ROUTES.en.autoCalibration },
  },
...
export default async function EnAutoCalibrationPage() {
  const { data } = await sanityFetch({
    ...
    params: { category: "equipment-systems-auto-calibration" },
  })
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="bg-white py-20 px-8">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-xs font-bold text-primary-700 tracking-widest uppercase mb-3">PRODUCT OVERVIEW</p>
              ...
              <div className="flex gap-3">
                <Link
                  ...
                  className="inline-flex px-5 py-2.5 bg-primary-700 text-white text-sm font-semibold rounded-lg hover:bg-accent transition-colors"
                >
                  Contact Us
                </Link>
                <Link
                  href={`${ROUTES.blog.cases}?industry=electronics`}
                  className="inline-flex px-5 py-2.5 border border-primary-700 text-primary-700 text-sm font-semibold rounded-lg hover:bg-primary-50 transition-colors"
                >
                  View Case Studies
                </Link>
              </div>
      ...
      <section className="bg-gray-50 py-20 px-8">
        <div className="max-w-5xl mx-auto">
          <p className="text-xs font-bold text-primary-700 tracking-widest uppercase mb-2">CORE FUNCTIONS</p>
            ...
            {FEATURES.map((f) => (
              <div key={f.num} className="bg-white rounded-2xl p-7 border border-gray-100 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-xs font-bold text-primary-700/40 tracking-widest">{f.num}</span>
                  <div className="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center text-primary-700">
                    {f.icon}
                  </div>
                </div>
                <p className="text-[10px] font-bold text-primary-700/60 tracking-widest uppercase mb-1">{f.en}</p>
      ...
      {/* ── Expected results ── */}
      <section className="bg-primary-700 text-white py-20 px-8">
        <div className="max-w-5xl mx-auto">
      ...
      {products.length > 0 && (
        <section className="bg-white py-20 px-8">
          <div className="max-w-5xl mx-auto">
            <p className="text-xs font-bold text-primary-700 tracking-widest uppercase mb-2">PRODUCTS</p>
            ...
            <ContentCardGrid isEmpty={false}>
              {products.map((p) => (
                <ContentCard
                  ...
                  href={`${ROUTES.solutions.equipmentSystem.autoCalibration}/${p.slug}`}
                />
              ))}
            </ContentCardGrid>
          </div>
