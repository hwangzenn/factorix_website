...
export default async function ResearchCasePage() {
  const { data } = await sanityFetch({ query: caseStudiesByCategoryQuery, params: { category: "research" } })
  const items = (data as CaseStudySummary[]) ?? []

  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <p className="text-sm text-primary-600 font-medium mb-2">적용사례 · 산업별</p>
      <h1 className="text-4xl font-bold text-primary-800 mb-6">연구기관/대학</h1>

      <IndustrySubNav active="research" />

      <ContentCardGrid isEmpty={items.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {items.map((item) => (
          <ContentCard key={item._id} title={item.title} description={item.description} thumbnailUrl={item.thumbnail?.asset?.url} thumbnailAlt={item.thumbnail?.alt} href={`${ROUTES.cases.industry.research}/${item.slug}`} />
        ))}
      </ContentCardGrid>
