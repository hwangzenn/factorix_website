...
export default async function BioCasePage() {
  const { data } = await sanityFetch({
    query: caseStudiesByCategoryQuery,
    params: { category: "bio" },
  })
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      ...
      <h1 className="text-4xl font-bold text-primary-800 mb-6">바이오</h1>

      <IndustrySubNav active="bio" />
      ...
      <ContentCardGrid isEmpty={items.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {items.map((item) => (
          <ContentCard
            ...
            href={`${ROUTES.cases.industry.bio}/${item.slug}`}
          />
        ))}
      </ContentCardGrid>
