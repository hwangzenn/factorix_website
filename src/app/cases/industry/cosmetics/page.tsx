...
export default async function CosmeticsCasePage() {
  const posts = await getPostsByTag("case-cosmetics");

  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <p className="text-sm text-primary-600 font-medium mb-2">적용사례 · 산업별</p>
      <h1 className="text-4xl font-bold text-primary-800 mb-10">화장품/뷰티</h1>
      {posts.length === 0 ? (
        ...
        <ul className="space-y-6 mb-16">
          {posts.map((post) => (
            <li key={post.id} className="border-b border-gray-100 pb-6">
              <Link href={`/cases/industry/cosmetics/${post.slug}`} className="text-lg font-semibold text-gray-800 hover:text-primary-700">{post.title}</Link>
