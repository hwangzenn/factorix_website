...
export default async function CosmeticsCaseDetailPage({ params }: Props) {
  const { slug } = await params
  const { data } = await sanityFetch({ query: caseStudyBySlugQuery, params: { slug } })
  const item = data as CaseStudyDetail | null
  if (!item) notFound()
  ...
  return (
    <ContentDetail
      eyebrow="적용사례 · 산업별 · 화장품/뷰티"
      backHref={ROUTES.cases.industry.cosmetics}
      backLabel="화장품/뷰티 적용사례"
      data={item}
    />
  )
}
