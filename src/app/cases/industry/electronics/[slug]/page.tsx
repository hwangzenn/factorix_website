...
export default async function ElectronicsCaseDetailPage({ params }: Props) {
  const { slug } = await params
  const { data } = await sanityFetch({ query: caseStudyBySlugQuery, params: { slug } })
  const item = data as CaseStudyDetail | null
  if (!item) notFound()
  ...
  return (
    <ContentDetail
      eyebrow="적용사례 · 산업별 · 전기/전자"
      backHref={ROUTES.cases.industry.electronics}
      backLabel="전기/전자 적용사례"
      data={item}
    />
  )
}
