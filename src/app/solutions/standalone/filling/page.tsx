...
export const metadata: Metadata = {
  ...
  alternates: { canonical: ROUTES.solutions.standalone.filling },
}

export default async function FillingPage() {
  const { data } = await sanityFetch({
    query: productsByCategoryQuery,
    params: { category: "standalone-filling" },
  })
      ...
      <ContentCardGrid isEmpty={products.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {products.map((p) => (
          <ContentCard
            ...
            href={`${ROUTES.solutions.standalone.filling}/${p.slug}`}
          />
        ))}
      </ContentCardGrid>
    </div>
