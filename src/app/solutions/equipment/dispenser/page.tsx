...
export const metadata: Metadata = {
  ...
  alternates: { canonical: ROUTES.solutions.equipment.dispenser },
}

export default async function DispenserPage() {
  const { data } = await sanityFetch({
    query: productsByCategoryQuery,
    params: { category: "equipment-dispenser" },
  })
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-10">
        <div>
          <CategoryBreadcrumb segments={["솔루션", "액상제조 장비", "디스펜서"]} />
          <h1 className="hidden sm:block text-4xl font-bold text-primary-800">디스펜서</h1>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <Link href={`${ROUTES.blog.cases}?industry=electronics`} className="inline-flex px-5 py-2.5 border border-primary-700 text-primary-700 text-sm font-semibold rounded-md hover:bg-primary-50 transition-colors">적용사례 보기</Link>
        </div>
      </div>
      ...
      <ContentCardGrid isEmpty={products.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {products.map((p) => (
          <ContentCard
            ...
            href={`${ROUTES.solutions.equipment.dispenser}/${p.slug}`}
          />
        ))}
      </ContentCardGrid>
    </div>
