...
export const metadata: Metadata = {
  ...
  alternates: { canonical: ROUTES.solutions.equipment.robot },
}

export default async function RobotPage() {
  const { data } = await sanityFetch({
    query: productsByCategoryQuery,
    params: { category: "equipment-robot" },
  })
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-10">
        <div>
          <CategoryBreadcrumb segments={["솔루션", "액상제조 장비", "협동/직교/3축로봇"]} />
      ...
      <ContentCardGrid isEmpty={products.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {products.map((p) => (
          <ContentCard
            ...
            href={`${ROUTES.solutions.equipment.robot}/${p.slug}`}
          />
        ))}
      </ContentCardGrid>
    </div>
