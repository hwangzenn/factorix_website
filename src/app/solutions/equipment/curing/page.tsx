...
export default async function CuringPage() {
  const { data } = await sanityFetch({
    ...
    params: { category: "equipment-curing" },
  })
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-10">
        <div>
          <CategoryBreadcrumb segments={["솔루션", "액상제조 장비", "UV/IR 경화기"]} />
          <h1 className="hidden sm:block text-4xl font-bold text-primary-800">UV/IR 경화기</h1>
        </div>
      ...
      <ContentCardGrid isEmpty={products.length === 0} emptyMessage="콘텐츠 준비 중입니다.">
        {products.map((p) => (
          <ContentCard
            ...
            href={`${ROUTES.solutions.equipment.curing}/${p.slug}`}
          />
        ))}
      </ContentCardGrid>
    </div>
