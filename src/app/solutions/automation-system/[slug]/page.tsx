...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${ROUTES.solutions.automationSystem}/${slug}` },
...
export default async function AutomationSystemDetailPage({ params }: Props) {
  const { slug } = await params
  const { data } = await sanityFetch({ query: productBySlugQuery, params: { slug } })
  const item = data as ProductDetail | null
  if (!item) notFound()
  ...
  return (
    <ContentDetail
      ...
      backHref={ROUTES.solutions.automationSystem}
      data={item}
