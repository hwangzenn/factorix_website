...
const GROUP_LABEL = "AI 자동보정 토출시스템"
const PROCESS_KEY = "dispenser"
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${ROUTES.solutions.equipmentSystem.autoCalibration}/${slug}` },
...
export default async function AutoCalibrationDetailPage({ params }: Props) {
  const { slug } = await params
  const { data } = await sanityFetch({ query: productBySlugQuery, params: { slug } })
  const item = data as ProductDetail | null
  if (!item) notFound()
  ...
  return (
    <ContentDetail
      ...
      backHref={ROUTES.solutions.equipmentSystem.autoCalibration}
      data={item}
